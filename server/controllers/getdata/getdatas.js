...
module.exports = async (req,res) => {
    ...
    for (let i =0; i < req.body.length; i++){
        ...
        const shopinfo = await shop.findOne({
            where : {
                location : req.body[i].road_address_name
            }
        })
        ...
        if (shopinfo){
            ...
            result[req.body[i].place_name]= {shoppic : photodatas, menulist : menulist };
            ...
            let photodatas = [] //이미지 크롤링 결과
            let menulist = {} //메뉴 정보 크롤링 결과
            let genus = req.body[i].category_name.split(" > ")[1]
            ...
            await page.goto(req.body[i].place_url);
            ...
            let content = await page.content(req.body[i].place_url);
            
            const $ = cheerio.load(content);
            const photolists = await $("#mArticle > div.cont_photo > div.photo_area > ul >li").children("a");
            const menulists = await $("#mArticle > div.cont_menu > ul >li > div").children("span");
            const price = await $("#mArticle > div.cont_menu > ul > li > div ").children("em.price_menu")
            const worktime = await $("#mArticle > div.cont_essential > div.details_placeinfo > div:nth-child(3) > div > div.location_present > ul > li > span").children("span")
            ...
            // 음식점 기본 정보 저장
            await shop.create({
                shop_name : req.body[i].place_name,
                genus : genus,
                location : req.body[i].road_address_name,
                work_time: work_time
            })
